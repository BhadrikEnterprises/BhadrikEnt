...
import { Save, Trash2, Check, FileSpreadsheet, UserCheck } from 'lucide-react';
import { exportClientStatementCSV } from '../lib/csv';
...
export function Settings() {
  ...
  const handleExportClientStatement = () => {
    ...
    exportClientStatementCSV(selectedClientId, data);
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto p-6">
      ...
      {/* Client Wise Statement Export Card */}
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-1">
          <UserCheck size={18} className="text-emerald-600" />
          <h2 className="text-base font-semibold text-slate-900">Client Statement Ledger Export</h2>
        </div>
        <p className="text-xs text-slate-500 mb-4">
          Select a borrower to generate and download a consolidated statement CSV containing all their disbursed loans and repayments chronologically.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <select
            ...
            className="flex-1 rounded-xl border border-slate-200 px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
          >
          ...
          <button
            onClick={handleExportClientStatement}
